...
class Moyasar {
  ...
  static Future<dynamic> pay({required String apiKey, required PaymentRequest paymentRequest}) async {
    final headers = buildRequestHeaders(apiKey);
    final body = jsonEncode(paymentRequest.toJson());

    final res = await http.post(Uri.parse(apiUrl), headers: headers, body: body);
